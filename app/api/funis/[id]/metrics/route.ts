...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const codigosProduto = funil.produtos.map((p) => p.codigoHotmart);

    const vendas = await getVendas({ dataInicio: inicio, dataFim: fim, codigosProduto });

    const currentMes = format(new Date(), "yyyy-MM");
    const gastoManualDoMes = funil.gastoManual.find((g) => g.mes === currentMes);
    ...
    const metricas = calcularMetricas(vendas, funil.produtos, fbData);
    const saude = calcularSaude(metricas, funil.historico, funil.produtos[0]?.metaMensal || 0);
